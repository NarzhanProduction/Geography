...
func (r *userRepository) Create(ctx context.Context, user *models.User) error {
	query := `INSERT INTO users (name, email, passwordHash) VALUES ($1, $2, $3) RETURNING id`
...
func (r *userRepository) GetByID(ctx context.Context, uuid uuid.UUID) (*models.User, error) {
	...
	err := r.db.QueryRow(ctx, query, uuid).Scan(&user.ID, &user.Name, &user.Email)
	if err != nil {
...
func (r *userRepository) GetByLogin(ctx context.Context, login string) (*models.User, error) {
	...
	err := r.db.QueryRow(ctx, query, login).Scan(&user.ID, &user.Name, &user.Email)
	if err != nil {
		return nil, fmt.Errorf("user not found: %w", err)
	}
	return user, nil
